package com.sulake.habbo.widget.furniture.dimmer
{
   ...
   public class DimmerViewColorGrid
   {
      ...
      private var var_442:BitmapData;
      
      private var var_1128:XML;
      ...
      private function populateColourGrid(param1:IHabboWindowManager) : void
      {
         ...
         var _loc13_:* = null;
         var _loc14_:* = null;
         var_374.destroyGridItems();
         var_297 = null;
         for each(_loc2_ in colors)
         {
            _loc3_ = param1.buildFromXML(var_1128) as IWindowContainer;
            _loc3_.addEventListener(WindowMouseEvent.WINDOW_EVENT_MOUSE_CLICK,onClick);
            _loc3_.background = true;
            _loc3_.color = 4294967295;
            _loc3_.width = var_442.width;
            _loc3_.height = var_442.height;
            var_374.addGridItem(_loc3_);
            _loc4_ = _loc3_.findChildByTag("BG_BORDER") as IBitmapWrapperWindow;
            if(_loc4_ != null)
            {
               _loc4_.bitmap = new BitmapData(var_442.width,var_442.height,true,0);
               _loc4_.bitmap.copyPixels(var_442,var_442.rect,new Point(0,0));
            }
            _loc5_ = _loc3_.findChildByTag("COLOR_IMAGE") as IBitmapWrapperWindow;
            if(_loc5_ != null)
            {
               _loc5_.bitmap = new BitmapData(var_759.width,var_759.height,true,0);
      ...
      {
         var _loc2_:int = var_374.getGridItemIndex(param1.target as IWindow);
      ...
      public function setSelectedColorIndex(param1:int) : void
      {
         ...
         if(param1 < 0 || param1 >= var_374.numGridItems)
         {
            return;
         }
         select(var_374.getGridItemAt(param1) as IWindowContainer);
      }
